...
export class AddbookComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._activeRoute.paramMap.subscribe((params) => {
      this.form.patchValue({ id: params.get('id') });
    });
  }
  ...
  send() {
    if (this.form.invalid) {
      ...
      const formControlDescription = this.form.get(
        'description'
      ) as FormControl;
      ...
      const formControlpagesNumber = this.form.get(
        'pagesNumber'
      ) as FormControl;
      ...
      debugger;
      const duplicatedBookId = cacheBooks.filter(
        (book: any) => book.id === newBook.id
      );
